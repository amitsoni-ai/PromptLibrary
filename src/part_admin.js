  ...
  function paintEditor() {
    ...
    body.querySelector("#col-save").addEventListener("click", () => {
      ...
      Backend.adminSaveCollection(d).then(() => { showToast("Collection saved"); st.editing = null; load(); })
        .catch(() => { btn.disabled = false; btn.textContent = d.id ? "Save changes" : "Create collection"; showToast("Couldn't save to the server"); });
    });
...
function renderAdminUsers(body) {
  ...
  function paintDrawer() {
    ...
    Promise.all([
      ...
      const act = (action, extra, confirmMsg) => {
        if (confirmMsg && !confirm(confirmMsg)) return;
        Backend.adminUserAction(uid, action, extra).then(() => { showToast("Done"); paintDrawer(); }).catch(() => showToast("Action failed"));
      };
      const entAct = (action) => Backend.adminEntitlementAction({ action, userId: uid }).then(() => { showToast(action); paintDrawer(); }).catch(() => showToast("Failed"));
      target.innerHTML = `
        <div class="section-title" style="margin-bottom:4px;"><h2 style="font-size:18px;">${escapeHtml(([u.firstName, u.lastName].filter(Boolean).join(" ")) || u.email)}</h2>
          <span class="chip ${u.accountStatus === "active" ? "chip-accent" : ""}">${escapeHtml((u.accountStatus || "").replace("_", " "))}</span></div>
        ...
        <div class="admin-drawer-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:18px;">
          <div>
            <h3 style="font-size:13px;margin:0 0 6px;">Profile</h3>
          ...
          <div>
            <h3 style="font-size:13px;margin:0 0 6px;">Access</h3>
            <div style="font-size:12.5px;color:var(--text-muted);margin-bottom:8px;">
              scope <b>${escapeHtml(acc.scopeType || "none")}</b> · ${acc.active ? "active" : "inactive"} · source ${escapeHtml(acc.source || "—")}<br>
              ${grantedN} feature${grantedN === 1 ? "" : "s"} granted · ${(acc.categoryIds || []).length} categories · ${(acc.programIds === "*" ? "all" : (acc.programIds || []).length)} programs
            </div>
            <div style="display:flex;gap:6px;flex-wrap:wrap;">
              <button class="btn btn-sm" id="dr-align">Align to function</button>
              <button class="btn btn-sm btn-ghost" data-ent="suspend">Suspend ent.</button>
              <button class="btn btn-sm btn-ghost" data-ent="expire">Expire</button>
              <button class="btn btn-sm btn-ghost" data-ent="revoke">Revoke</button>
              <button class="btn btn-sm btn-ghost" data-ent="reactivate">Reactivate</button>
            </div>
            <div style="margin-top:8px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
              <select id="dr-assign-scope"><option value="full">full</option><option value="function">function</option><option value="program">program</option><option value="collection">collection</option><option value="none">none</option></select>
              <input id="dr-assign-progs" placeholder="program ids (comma)" style="min-width:160px;"/>
              <button class="btn btn-sm btn-primary" id="dr-assign">Assign entitlement</button>
            </div>
          </div>
        </div>

        <div style="display:flex;gap:6px;flex-wrap:wrap;margin:16px 0;">
          <button class="btn btn-sm btn-ghost" data-ua="suspend">Suspend acct</button>
          <button class="btn btn-sm btn-ghost" data-ua="reactivate">Reactivate</button>
          <button class="btn btn-sm btn-ghost" data-ua="disable">Disable</button>
          <button class="btn btn-sm btn-ghost" data-ua="resend_verification">Resend verify</button>
          <button class="btn btn-sm btn-ghost" data-ua="force_verify">Force verify</button>
          <button class="btn btn-sm btn-ghost" data-ua="send_reset">Send reset</button>
          <button class="btn btn-sm btn-ghost" data-ua="revoke_sessions">Revoke sessions</button>
          <div style="flex:1;"></div>
          <button class="btn btn-sm btn-ghost" id="dr-softdel" style="color:var(--danger);">Soft delete</button>
          ${isSuperAdmin() ? `<button class="btn btn-sm btn-ghost" id="dr-harddel" style="color:var(--danger);">Hard delete</button>` : ""}
        </div>
      ...
      target.querySelector("#dr-save-profile").addEventListener("click", () => act("update_profile", {
      ...
      target.querySelector("#dr-align").addEventListener("click", () => act("align_function", {}, "Re-scope this user's auto entitlement to their current function?"));
      target.querySelectorAll("[data-ent]").forEach((b) => b.addEventListener("click", () => entAct(b.dataset.ent)));
      target.querySelector("#dr-assign").addEventListener("click", () => {
        const scopeType = target.querySelector("#dr-assign-scope").value;
        const programIds = target.querySelector("#dr-assign-progs").value.split(/[\s,]+/).map((s) => s.trim()).filter(Boolean);
        Backend.adminEntitlementAction({ action: "assign", userId: uid, scopeType, programIds }).then(() => { showToast("Entitlement assigned"); paintDrawer(); }).catch(() => showToast("Failed"));
      });
      target.querySelectorAll("[data-ua]").forEach((b) => b.addEventListener("click", () => act(b.dataset.ua, { reason: "admin console" }, b.dataset.ua === "disable" ? "Disable this account?" : null)));
      target.querySelectorAll("[data-unenroll]").forEach((b) => b.addEventListener("click", () => {
        Backend.adminEntitlementAction({ action: "unenroll", userId: uid, programId: b.dataset.unenroll }).then(() => { showToast("Unenrolled"); paintDrawer(); }).catch(() => showToast("Failed"));
      }));
      target.querySelector("#dr-softdel").addEventListener("click", () => {
        if (!confirm("Soft-delete (disable + revoke sessions) this account?")) return;
        Backend.adminUserDelete(uid, false).then(() => { showToast("Account disabled"); st.drawer = null; load(); }).catch(() => showToast("Failed"));
      });
      ...
      if (hd) hd.addEventListener("click", () => {
        ...
        Backend.adminUserDelete(uid, true).then(() => { showToast("User hard-deleted"); st.drawer = null; load(); }).catch(() => showToast("Failed"));
      });
