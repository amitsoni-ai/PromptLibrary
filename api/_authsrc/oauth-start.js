...
export default async function handler(req, res) {
  ...
  if (!providerEnabled(key)) return redirect(res, "/login?oauth_error=unavailable");

  const p = PROVIDERS[key];
  const state = randomToken(18);
  const verifier = randomToken(48);
  const nonce = randomToken(18);
  const payload = Buffer.from(JSON.stringify({ p: key, s: state, v: verifier, n: nonce })).toString("base64url");
