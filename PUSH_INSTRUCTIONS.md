...
`Synottic_Logo.png` (repo root) is the master mark; `build.py` inlines it as a data-URI for the
in-app logo and favicon. `favicon.png` / `apple-touch-icon.png` / `og-image.png` (repo root) are
derived by `src/make-assets.sh` (needs Chrome) and referenced by `index.html` for the browser
tab and social preview. Regenerate: `cd src && ./make-assets.sh && python3 build.py`.
...
The app now ships a `/api` serverless layer + Neon Postgres. With no `DATABASE_URL` it
degrades to browser localStorage (today's behaviour). Setup — including the migration that
seeds Neon from the data parts — is in **`BACKEND.md`**.

## Access codes

Full list in **`ACCESS-CODES.md`** (generated by `src/curriculum.py`).

### Access tiers
| Tier | Code | Sees |
|---|---|---|
| Super-admin | `SYNOTTIC-SUPERADMIN` | Whole library **+** the admin console (an "Admin console" link appears in the app). |
| Admin console only | `SYNOTTIC-ADMIN` | The admin console (entered on the sign-in screen). |
| Global / all-roles | `SYNOTTIC-ALL` (or `DEMO-2026`) | Whole library — Categories & Library Governance tabs visible. |
| Scoped learner | every other `SYNOTTIC-<TOPIC>` / `SYNOTTIC-<TRACK>` code | Only their program's prompts. **Categories and Library Governance are hidden** — they discover via My Program, search, Learn and Practice. |


Every code is in **`ACCESS-CODES.md`** (generated by `src/curriculum.py`): the admin key,
7 whole-track codes, one `SYNOTTIC-<TOPIC>` code per course (44), and the demo/client codes.
All `SYNOTTIC-*` course codes are pre-seeded into the Admin console and are editable there.
